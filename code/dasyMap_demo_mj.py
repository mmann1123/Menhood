                ...
                for normalField in agg_normalField:
                    ...
                    for i in agg_df.GEO_ID.unique():
                        ...
                        field_weightsum = pd.DataFrame(
                                        ...
                                        {
                                            "GEO_ID":i,
                             				normalField:(agg_df[agg_df.GEO_ID == i][normalField].replace(0.00, np.nan).astype(float) *
                                            (agg_df[agg_df.GEO_ID == i]["PERCENTAGE"]/100)).sum()
                                        },
                                        index = [0]
                                    )

                        print field_weightsum

                        agg_normalField_list.append(field_weightsum)
                    ...
                    for i in agg_df.GEO_ID.unique():
                        ...
                        field_avg = pd.DataFrame(

                            {
                             ...
                             "GEO_ID":i,
                              countField:agg_df[agg_df.GEO_ID == i][countField].replace(0.00, np.nan).astype(float).mean()

                            }, index =  [0]
                        )

                        agg_countField_list.append(field_avg)
